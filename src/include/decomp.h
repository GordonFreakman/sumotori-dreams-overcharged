...
#define DECOMP_STATIC_ASSERT_VALUE(value, line)                                \
  DECOMP_STATIC_ASSERT_VALUE2(value, line)
#define DECOMP_STATIC_ASSERT(value) DECOMP_STATIC_ASSERT_VALUE(value, __LINE__)
#if (defined(_WIN64) ||                                                        \
     (defined(__SIZEOF_POINTER__) && __SIZEOF_POINTER__ == 8))
#define DECOMP_SIZE_ASSERT(type, size)                                         \
  DECOMP_STATIC_ASSERT(sizeof(type) >= (size))
#else
#define DECOMP_SIZE_ASSERT(type, size)                                         \
  DECOMP_STATIC_ASSERT(sizeof(type) == (size))
#endif
