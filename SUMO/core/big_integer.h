...
public:
  ...
  BigInteger operator-(BigInteger& other);
  BigInteger operator*(BigInteger& other);
  __declspec(nothrow) BigInteger operator<<(SumoS32 bits);
  BigInteger operator/(BigInteger& other);
  BigInteger operator%(BigInteger& other);
  ...
  void ModularPower(BigInteger& base, BigInteger& exponent, BigInteger& modulus);
...
private:
  friend void Divide(BigInteger* dividend, BigInteger* divisor, BigInteger* quotient);
